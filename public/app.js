...
function showHome(user) { hideAll(); esconderChat(); limparMensagensChat(); document.getElementById('home-screen').classList.add('active'); isSpectator = false; const name = user.displayName || user.username || 'Jogador'; const photo = ((user.foto_url || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(user.username) + '&background=6c63ff&color=fff&size=80') && (user.foto_url || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(user.username) + '&background=6c63ff&color=fff&size=80') ? (user.foto_url || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(user.username) + '&background=6c63ff&color=fff&size=80').value : ''; document.getElementById('welcome-name').textContent = (user.role === 'admin' ? '👑 ' : '') + name; document.getElementById('user-name').textContent = (user.role === 'admin' ? '👑 ' : '') + name; const avatar = document.getElementById('user-avatar'); if (photo) { avatar.src = photo; avatar.style.display = 'block'; } myStatus = 'alive'; meuPapel = 'cidadao'; }
