...
// Mostra botoes pos-jogo: Jogar Novamente e Sair da Sala

function mostrarBotoesPosJogo() {
...
if (btnSairPosJogo) btnSairPosJogo.style.display = 'inline-block';
...
// Host resetou a sala — todos voltam para tela de sala
...
if (isSpectator) {
const statusEl = document.getElementById('spectator-status');
const icon = data.tipo === 'reveal' ? '💀' : (data.vencedor === 'cidade' ? '🏆' : '💀');
const txt = data.tipo === 'reveal' ? 'Assassino Venceu!' : (data.vencedor === 'cidade' ? 'Cidade Venceu!' : 'Assassino Venceu!');
if (statusEl) statusEl.textContent = icon + ' ' + txt;
const btnVirar = document.getElementById('btn-virar-jogador');
if (btnVirar) btnVirar.style.display = 'block';
return;
}
// Mostra tela de decisao
hideAll();
...
rev.textContent = '🔪 ' + data.assassino.name + ' ERA O ASSASSINO!';
rev.className = 'role-reveal assassino';
} else {
...
document.getElementById('decision-reveal').style.display = 'none';
}
// Inicia countdown de decisao
...
restante--;
if (timerEl) timerEl.textContent = restante;
if (restante <= 0) clearInterval(decInterval);
}, 1000);
});

// ---- SALA PRONTA: volta para tela de sala (quem ficou) ----
...
document.getElementById('btn-sair-pos-jogo').disabled = true;
...
document.getElementById('decision-aguardando').style.display = 'block';
});
}
// Botao Sair pos-jogo
const btnSairPosJogo = document.getElementById('btn-sair-pos-jogo');
...
// Botao Voltar a Sala (home)
const btnVoltarSala = document.getElementById('btn-voltar-sala');
if (btnVoltarSala) {
btnVoltarSala.addEventListener('click', async () => {
if (!currentRoom) return;
const res = await fetch('/api/rooms/' + currentRoom.code + '/join', { method: 'POST', headers: { 'Content-Type': 'application/json' } });
const data = await res.json();
if (data.error) { alert('Sala nao encontrada. Ela pode ter sido encerrada.'); currentRoom = null; const homeAtiva = document.getElementById('home-sala-ativa'); if (homeAtiva) homeAtiva.style.display = 'none'; return; }
if (data.asSpectator) { showSpectatorRoom(data.room); } else { showRoom(data.room); }
});
}
// Botao Virar Jogador (espectador)
const btnVirarJogador = document.getElementById('btn-virar-jogador');
if (btnVirarJogador) {
btnVirarJogador.addEventListener('click', async () => {
if (!currentRoom) return;
const res = await fetch('/api/rooms/' + currentRoom.code + '/virar-jogador', { method: 'POST' });
const data = await res.json();
if (data.error) { alert(data.error); return; }
isSpectator = false;
currentRoom = data.room;
